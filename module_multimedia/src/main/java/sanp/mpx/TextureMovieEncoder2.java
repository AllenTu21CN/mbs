...
import sanp.javalon.media.video.VideoEncoderCore;
...
public class TextureMovieEncoder2 implements Runnable {
    ...
    public TextureMovieEncoder2(VideoEncoderCore encoderCore) {
        LogManager.d("Encoder: startRecording()");

        mVideoEncoder = encoderCore;

        synchronized (mReadyFence) {
            if (mRunning) {
                LogManager.w("Encoder thread already running");
                return;
            }
            mRunning = true;
    ...
    public void run() {
        ...
        LogManager.d("Encoder thread exiting");
        synchronized (mReadyFence) {
    ...
    private static class EncoderHandler extends Handler {
        ...
        public void handleMessage(Message inputMessage) {
            ...
            if (encoder == null) {
                LogManager.w("EncoderHandler.handleMessage: encoder is null");
                return;
            }
