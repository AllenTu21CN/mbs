...
public class MediaTesting implements MediaController.Observer {

    private static final String TAG = "MediaTesting";
    ...
    public static String TEST_1080P_24FPS_2MBPS_32SECS = "test_1080p_24fps_2mbps_32secs.mp4";
    public static String TEST_720P_24FPS_1MBPS_17SECS = "test_720p_24fps_1mbps_17secs.mp4";

    private static Map<VideoFormat, String> gDecodingFiles = new HashMap<VideoFormat, String>() {{
        put(VideoFormat.FORMAT_1080P_1M_25FPS, "rtsp://10.1.83.200:5000/main.h264");
        put(VideoFormat.FORMAT_1080P_2M_25FPS, TEST_1080P_24FPS_2MBPS_32SECS);
        put(VideoFormat.FORMAT_720P_1M_25FPS, TEST_720P_24FPS_1MBPS_17SECS);
    }};
    ...
    private static MediaTesting gMediaTesting = null;

    public static MediaTesting getInstance() {
        if (gMediaTesting == null) {
            synchronized (MediaTesting.class) {
                if (gMediaTesting == null) {
                    gMediaTesting = new MediaTesting();
                }
            }
        }
        return gMediaTesting;
    }

    private String mEncodingFilePath;
    private Map<VideoFormat, String> mDecodingFiles = new HashMap<>();
    ...
    private List<MediaController.Observer> mBackupObs = null;

    private MediaTesting() {
        if(InitMultimedia.ModuleHomePath == null)
            throw new RuntimeException("MediaTesting: call InitMultimedia.setAppHomePath first");

        for(Map.Entry<VideoFormat, String> entry: gDecodingFiles.entrySet()) {
            String url = entry.getValue();
            if(url.endsWith(".mp4")) {
                url = InitMultimedia.ModuleHomePath + "/" + url + "?loop=true";
            }
            mDecodingFiles.put(entry.getKey(), url);
        }
        mEncodingFilePath = InitMultimedia.ModuleHomePath + "/mt";
    }

    public void entryTesting() {
        new File(mEncodingFilePath).mkdir();
        mMediaController = MediaController.getInstance();
        mBackupObs = mMediaController.cleanObservers();
    ...
    public void exitTesting() {
        ...
        mMediaController.setObservers(mBackupObs);
        mBackupObs = null;
        cleanTmpDir();
    }
    ...
    public void cleanTmpDir() {
        FileUtils.deleteDir(mEncodingFilePath);
    }
    ...
    private void startMediaCompositiveTesting(int captureDevices, VideoFormat decodingFormat, int decodingCnt,List<VideoFormat> encodings, TestingType type) {
        ...
        // 添加解码源
        String decodingFile = mDecodingFiles.get(decodingFormat);
        ...
        for(VideoFormat format: encodings) {
            ...
            int id = mMediaController.addOutput(mEncodingFilePath + "/test-" + i + ".mp4", video, MediaController.RECOMMENDED_REOPEN_CNT);
            if(id >= 0)
    ...
    public void startEncodedTesting2() {
        String url = mEncodingFilePath + "/test-0.mp4" + "?loop=true";
        mTestingType = TestingType.PRODUCTION_TESTING_ENCODED;
        mMediaController.addSource(url,  Arrays.asList(AVDefines.DataType.VIDEO), MediaController.RECOMMENDED_REOPEN_CNT);
    }
    ...
    public static class Tester implements SimpleTesting.Tester {
        MediaTesting mMediaTesting = null;
        private int mTestingStep = -1;

        public void start(Object obj) {
            mMediaTesting = MediaTesting.getInstance();
            mMediaTesting.entryTesting();
            mMediaTesting.startMediaCompositiveTesting(CAPTURE_DEVICE_NONE, DEFAULT_1080P_FORMAT, 2, DEFAULT_1080P_FORMAT, 1);
            mTestingStep = 4;
        }

        public void next() {
            if (mTestingStep == 4) {
                mMediaTesting.stopMediaTesting();
            } else if (mTestingStep == 3) {
                mMediaTesting.cleanTmpDir();
            } else {
                if (mMediaTesting != null) {
                    mMediaTesting.exitTesting();
                    mMediaTesting = null;
                }
                return;
            }
