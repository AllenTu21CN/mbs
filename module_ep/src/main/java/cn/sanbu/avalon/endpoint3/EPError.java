...
public enum  EPError {
    ...
    EPError(CallingProtocol protocol, int code, String reason, String hint, boolean retry) {
    ...
    public static EPError fromCode(int code) {
        for (EPError error: EPError.values()) {
            if (code == error.code)
                return error;
        }
        return UNKNOWN;
    ...
    public static EPError fromCode2(CallingProtocol protocol, int code, String reason) {
        for (EPError error: EPError.values()) {
            if (code == error.code &&
