...
import cn.lx.media.gles.Drawable2d;
import cn.lx.media.gles.EglCore;
import cn.lx.media.gles.FullFrameRect;
import cn.lx.media.gles.FlatShadedProgram;
import cn.lx.media.gles.SharedTextureManager;
import cn.lx.media.gles.Sprite2d;
import cn.lx.media.gles.Texture2dProgram;
import cn.lx.media.gles.GlUtil;
import cn.lx.media.gles.ScaledDrawable2d;
import cn.lx.media.gles.OffscreenSurface;
import cn.lx.media.gles.TransitionProgram;
import cn.lx.media.gles.WindowSurface;
...
public class VideoEngine {
    ...
    //private volatile boolean[] mHdmiInputDeviceHasSignal = new boolean[2];
    ...
    private class Source implements Runnable {
        ...
        private volatile boolean mHasSignal;
        ...
        private class Stats {
            ...
            private Stats() {
                if (mType == SOURCE_TYPE_CAPTURE) {
                    ...
                    codec       = "";
                    bitrate     = -1;
                    frame_rate  = -1;   // TODO: Get capture fps
        ...
        public Source(int id, int type, String config) {
            Log.i(TAG, "Initializing video source id=" + id
        ...
        protected void finalize() throws Throwable {
            try {
                Log.i(TAG, "Finalizing VideoEngine::Source object. id=" + mId);
            } finally {
                super.finalize();
            }
        }
        ...
        public boolean init() {
            ...
            switch (mType) {
                ...
                case SOURCE_TYPE_CAPTURE:
                    ...
                    if (configObject.has(CAPTURE_WIDTH_KEY)
                        ...
                        try {
                            ...
                            Log.w(TAG, "Parse capture size parameters failed!");
                        }
                    }
                ...
                default :
                    Log.e(TAG, "Unsupported video source type " + mType);
                    return false;
            }
        ...
        private boolean enqueuePacket(byte[] packet) {
            if (mType != SOURCE_TYPE_DECODER) {
                Log.e(TAG, "Current source type doesn't support enqueuePacket");
                return false;
            }
        ...
        private boolean attachDisplaySurface(Surface surface) {
            if (SOURCE_TYPE_CAPTURE != mType) {
                Log.e(TAG, "Decoder type video source on Android platform does not support this operation.");
                return false;
            }
        ...
        private void closeCamera() {
            ...
            //int port = HdmiInputDevice.cameraIdToHdmiDeviceId(mCameraId);
        ...
        private void reopenCamera() {
            ...
            if (mCaptureSizeAutoFit) {
                ...
                //HdmiInputDevice.Format fmt = mHdmiInputDevice.queryFormat(port);
                //if (fmt == null) {
                //    return;
                //}
        ...
        private void createCameraPreviewSession() {
            Log.i(TAG, "Creating camera preview session.");
                                    ...
                                    mCameraCaptureSession.setRepeatingRequest(
                                            mCameraPreviewRequest, new CameraCaptureSession.CaptureCallback() {
                                                ...
                                                public void onCaptureFailed(@NonNull CameraCaptureSession session,
                                                    ...
                                                    Log.e(TAG, "CameraCaptureSession.onCaptureFailed");
                                                    // TODO:
                                                }
        ...
        private void destroyCameraPreviewSession() {
            Log.i(TAG, "Destroying camera preview session.");
            ...
            mOutputSurfaceTexture.setOnFrameAvailableListener(
                    new SurfaceTexture.OnFrameAvailableListener() {
                        ...
                        public void onFrameAvailable(SurfaceTexture surfaceTexture) {
                            //Log.d(TAG, "SurfaceTexture.OnFrameAvailableListener.onFrameAvailable");
        ...
        public void statFrameRate() {
            ...
            synchronized (mStatFence) {
                ...
                boolean hasSignal = (last_pts > 0 && now - last_pts < NO_SIGNAL_THRESHOLD);

                // Update HDMI Input Device is plugged status table
                if (mHasSignal != hasSignal && SOURCE_TYPE_CAPTURE == mType) {
                    //int port = HdmiInputDevice.cameraIdToHdmiDeviceId(mCameraId);
                    //mHdmiInputDeviceHasSignal[port] = hasSignal;
                    Log.d(TAG, "Source id=" + mId + ", cam_id=" + mCameraId + " has signal: " + hasSignal);
                }
        ...
        public void run() {
            //Log.d(TAG, "Video source #" + mId + " thread started!");
            ...
            Packet packet = null;
            Log.d(TAG, "Enter video decoding loop.");
            while (!outputDone) {
                // Check stop flag
                if (mStopFlag) {
                    Log.i(TAG, "VideoSource#" + mId + " thread exited!");
                ...
                if (!inputDone) {
                    do {
                        if (packet == null) {
                            ...
                            //Log.d(TAG, "Unserialized packet pts=" + packet.pts
                        ...
                        if (inputBufIndex < 0) {
                            Log.i(TAG, "Decoder input buffer is full, try later. source#" + mId);
                            break;
                        }
                        ...
                        try {
                            ...
                            Log.w(TAG, "Input buffer is not large enough (packet size="
                                    + packet.size + ")");
                        }
        ...
        private void updateTexture() {
            if (SOURCE_TYPE_CAPTURE == mType && null == mCameraDevice) {
                //Log.e(TAG, "Video source #" + mId + " camera closed!");
            ...
            synchronized (mUpdateSurfaceLock) {
                if (mUpdateSurface) {
                    synchronized (mDrawFence) {
                        ...
                        s += "]";
                        Log.d(TAG, "Output transform matrix: " + s);*/
                    }
    ...
    private class Scene {
        ...
        private class Config implements Cloneable {
            ...
            public void release() {
                ...
                if (displayNameTexSlot >= 0) {
                    //Log.d(TAG, "Config release display name texture slot, id=" + displayNameTexSlot);
            ...
            /* public void setBackgroundImage(String imagePath) {
                ...
                if (imagePath.equals("none")) {
                    Log.i(TAG, "Disable background image for scene #" + mId);
                    mBackgroundImage = null;
                    return;
                }

                // Load image from file and convert into byte buffer
                Log.i(TAG, "Set background image(" + imagePath + ") for scene #" + mId);
                mBackgroundImage = BitmapFactory.decodeFile(imagePath);
                if (mBackgroundImage != null) {
                    Log.d(TAG, "Loaded background image: width=" + mBackgroundImage.getWidth()
                    ...
                    Log.e(TAG, "Loading background image failed!");
                    // TODO: Notify
                }
            } */
            ...
            private void updateDisplayNameImageBuffer() {
                Log.i(TAG, "Updating display name image buffer for scene #" + mId);
        ...
        private class Overlay {
            ...
            private Overlay(Bitmap image) {
                ...
                if (image != null && image.getWidth() > 0 && image.getHeight() > 0) {
                    ...
                    //Log.d(TAG, "Overlay request an texture slot for image, id=" + mImageTexSlot);

                    /*mImageTexCoordArray = mSharedTexMgr.getTexCoordArray(mImageTexSlot, mImageWidth, mImageHeight);*/
                    //Log.i(TAG, "Allocated shared texture properties: slot="
            ...
            private void drawDisplayName() {
                ...
                if (mNeedUpdateDisplayNameBitmap) {
                    ...
                    if (mDisplayNameTexSlot < 0) {
                        ...
                        //Log.d(TAG, "Overlay request an texture slot for display name, id=" + mDisplayNameTexSlot);
                    }
            ...
            private void release() {
                if (OVERLAY_TYPE_IMAGE == mType && mImageTexSlot >= 0) {
                    //Log.d(TAG, "Overlay release image texture slot, id=" + mImageTexSlot);
                    mSharedTexMgr.releaseSlot(mImageTexSlot);
                    mImageTexSlot = -1;
                }

                if (mDisplayNameTexSlot >= 0) {
                    //Log.d(TAG, "Overlay release display name texture slot, id=" + mDisplayNameTexSlot);
        ...
        public class Recorder {
            ...
            public Recorder(MixerThread mixer, Scene scene, Sink sink) {
                ...
                if (sink != null) {
                    ...
                    //} else {
                    //    Log.e(TAG, "Sink input surface is null!");
                    //}
            ...
            public void beginRecord() {
                ...
                if (scene != null && sink != null) {
                    //Log.d(TAG, "Mixer.Recorder beginRecord for sink #" + sink.getId());
            ...
            public void finishRecord(long timeStampNanos) {
                Sink sink = mWeakRefSink.get();
                if (sink != null) {
                    //Log.d(TAG, "Mixer.Recorder finishRecord for sink #" + sink.getId());
        ...
        private class Transition {
            ...
            private void set(String transitionType, float transitionDuration) {
                if (transitionType == null || transitionType.equals("none") || transitionDuration <= 0.0f) {
                    ...
                    //Log.d(TAG, "Transition activate! type=" + type + ", progress=" + progress);
                    ...
                    if (mTransitionProgram == null) {
                        ...
                        if (!mTransitionProgram.getType().equals(type)) {
                            ...
                            try {
                                ...
                                Log.e(TAG, "Allocate transition program failed! Skip configuration.");
                                mTransitionProgram = null;
                                progress = 1.0f;
                            }
                        }
            ...
            private void update(long timeStampNanos) {
                ...
                if (progress < 1.0f) {
                    if (progress > 0.0f) {
                        ...
                        //Log.d(TAG, "Transition running!, type=" + type + ", progress=" + progress);
                    } else {
                        mStartTimestampMs = timeStampNanos / 1000000;
                        progress = 0.001f;
                        //Log.d(TAG, "Transition started!, type=" + type + ", progress=" + progress);
                    }
        ...
        private class RenderThread extends Thread {
            ...
            public void run() {
                ...
                Log.i(TAG, "RenderThread for scene#" + scene.mId + " started!");
                ...
                Log.d(TAG, "VideoSceneRender-" + mId + " thread looper exited!");
                ...
                switch (msg.what) {
                    case MSG_RENDER : {
                        ...
                        if (scene != null) {
                            ...
                            //Log.d(TAG, "Video scene #" + mId + " rendered done!");
                        }
                        break;
                    }
        ...
        protected void finalize() throws Throwable {
            try {
                Log.i(TAG, "Finalizing VideoEngine::Scene object. id=" + mId);
            } finally {
                super.finalize();
            }
        }
            ...
            if (configObject.has(CONFIG_OVERLAYS_KEY)) {
                ...
                for (JsonElement overlay : overlaysArray) {
                    ...
                    if (!overlay.isJsonObject()) {
                        Log.e(TAG, "Overlay element is not an object.");
                        continue;
                    }
                        ...
                        if (type.equals("stream")) {
                            ...
                            if (overlayObject.has(CONFIG_IMAGE_PATH_KEY)) {
                                ...
                                if (!image_path.isEmpty()) {
                                    ...
                                    Log.e(TAG, "Empty overlay image_path.");
                                    continue;
                                }
                            }
                        }
                    } else {
                        Log.e(TAG, "Overlay object without type.");
                        continue;
                    }
            ...
            if (configObject.has(CONFIG_TRANSITION_KEY)) {
                ...
                if (transitionElement.isJsonObject()) {
                    ...
                    if (transitionObject.has(CONFIG_TRANSITION_TYPE_KEY)) {
                        ...
                        //Log.d(TAG, "Got transitionType: " + transitionType);
                    }
                    ...
                    if (transitionObject.has(CONFIG_TRANSITION_DURATION_KEY)) {
                        ...
                        //Log.d(TAG, "Got transitionDuration: " + transitionDuration);
                    }
                }
            }
        ...
        private void attachDisplaySurface(Surface surface) {
            if (mMixer.getEglCore() == null) {
                Log.e(TAG, "EGL Core is not initialized!");
                return;
            }

            mDisplaySurface = surface;
            if (mDisplaySurface == null) {
                Log.e(TAG, "Display surface is null, nothing to attach!");
            }
        ...
        private void onDisplaySurfaceChanged(int format, Size size) {
            Log.d(TAG, "Video scene #" + mId + " display surface changed, format: " + format
        ...
        private void detachDisplaySurface() {
            if (mDisplaySurface == null) {
                Log.e(TAG, "Display surface is null, nothing to detach!");
            } else {
        ...
        public void deleteSink(int sinkId) {
            ...
            for (Recorder recoder : mRecorders) {
                ...
                if (sink != null && sink.getId() == sinkId) {
                    Log.d(TAG, "Video sink id=" + sinkId + " recorder removed from mixing thread.");
                    recoder.release();
        ...
        private void release() {
            Log.d(TAG, "Release video scene #" + mId + " internal resources.");
            if (mEnableMultiThreadRender) {
                // TODO: Call releaseGL in render thread.
            } else {
                if (mSharedTexMgr != null) {
                    Log.i(TAG, "Release shared texture manager for scene#" + mId);
        ...
        private void prepareGL(EglCore eglCore) {
            Log.d(TAG, "Prepare Gl for video scene");
        ...
        private void prepareSharedTexture() {
            Log.i(TAG, "Allocate shared texture manager for scene#" + mId);
        ...
        private void releaseGL() {
            Log.d(TAG, "Release GL resources.");
        ...
        private void render(long timeStampNanos) {
            //Log.d(TAG, "Rendering scene #" + mId + "timestamp:" + timeStampNanos);
        ...
        private void drawDisplayName(Config config) {
            ...
            if (config.needUpdateDisplayNameBitmap) {
                ...
                if (config.displayNameTexSlot < 0) {
                    ...
                    //Log.d(TAG, "Config request an texture slot for display name, id=" + config.displayNameTexSlot);
                }
        ...
        private int parseAlignment(String alignment) {
            ...
            switch (horizontal) {
                ...
                default :
                    Log.e(TAG, "Invalid horizontal alignment type!");
                    return 0;
            }
            ...
            switch (vertical) {
                ...
                default :
                    Log.e(TAG, "Invalid vertical alignment type!");
                    return 0;
            }
    ...
    private class Sink implements Runnable {
        ...
        private class Config {
            ...
            public Config(String config) {
                ...
                if (configObject.has(CONFIG_LEVEL_KEY)) {
                    level = configObject.get(CONFIG_LEVEL_KEY).getAsInt();
                }
                ...
                if (configObject.has(CONFIG_RESOLUTION_KEY)) {
                    ...
                    if (resolution.toLowerCase().equals("auto")) {
                        // TODO:
                        Log.w(TAG, "Auto resolution NOT IMPLEMENTED yet. Using 1920x1080 instead.");
                    } else {
            ...
            public String getMimeType() {
                if (codec.equals("H264")) {
                    ...
                    Log.e(TAG, "Unsupported video codec " + codec);
                }
            ...
            public MediaFormat getMediaFormat() {
                ...
                if (mimeType.equals(MediaFormat.MIMETYPE_VIDEO_AVC)) {
                    switch (mConfig.profile.toLowerCase()) {
                        ...
                        case "high" :
                            ...
                            break;
                        default :
                            Log.e(TAG, "Invalid profile(" + profile + ") for codec(" + codec + ")");
                    }
                ...
                // Level
                int[] androidLevels = {
                    0,  // H264_PROFILE_LVL_UNSPECIFIED
                ...
                if (level > 0 && level < androidLevels.length) {
                    //if (!Rockchip.isProduct())
                        format.setInteger(MediaFormat.KEY_LEVEL, androidLevels[level]);
                }
        ...
        private class EncodeHandler extends Handler {
            ...
            public void handleMessage(Message inputMessage) {
                ...
                if (videoSink == null) {
                    Log.w(TAG, "EncodeHandler.handleMessage: sink is null");
                    return;
                }
        ...
        public Sink(int id, int inputType, int inputObjectId, String config) {
            ...
            try {
                ...
                Log.i(TAG, "Create timer for request key frame periodically.");
        ...
        protected void finalize() throws Throwable {
            try {
                Log.i(TAG, "Finalizing VideoEngine::Sink object. id=" + mId);
            } finally {
                super.finalize();
            }
        }
        ...
        public int getSceneId() {
            if (mInputType != INPUT_TYPE_SCENE) {
                Log.e(TAG, "Input type mismatched!");
                return -1;
            }
            return mSceneId;
        }
        ...
        public void release() {
            ...
            mBufferInfo = null;
            mConfig = null;
            Log.i(TAG, "Video sink id=" + mId + " released!");
        }
        ...
        public void run() {
            ...
            Log.i(TAG, "Video sink thread exited");
        }
        ...
        private void handleRequestKeyFrame() {
            Log.d(TAG, "Video sink id=" + mId + " requestKeyFrame.");
            ...
            if (now >= mLastKeyFrameTimeStamp + MIN_KEY_FRAME_INTERVAL) {
                ...
                try {
                    ...
                    Log.e(TAG, "Video sink id=" + mId + " requestKeyFrame failed: "
                            + e.getMessage());
                }
            } else {
                Log.i(TAG, "Video sink requestKeyFrame, got key frame request repeatedly in "
            ...
            while (true) {
                ...
                if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    if (mBufferInfo.presentationTimeUs < 0) {
                        Log.e(TAG, "!!! Sink error: pts from encoder is invalid "
        ...
        private void onEncodedData(ByteBuffer encodedData) {
            ...
            if ((mBufferInfo.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                Log.d(TAG, "Update IDR from encoder, sink_id=" + mId);
                mIDRPacket = packet;
            } else if (packet.isKeyFrame && mIDRPacket != null) {
                Log.d(TAG, "Prepend IDR ahead of key frame, sink_id=" + mId);
    ...
    private class MixerThread extends Thread {
        ...
        public void run() {
            ...
            Log.i(TAG, "[GLES] Maximum texture image units: " + maxTextureUnits[0]);

            int[] maxTextureSize = new int[1];
            GLES20.glGetIntegerv(GLES20.GL_MAX_TEXTURE_SIZE, maxTextureSize, 0);
            Log.i(TAG, "[GLES] Maximum texture size: " + maxTextureSize[0]);
            ...
            Log.d(TAG, "Mixer thread looper exited!");
            releaseGl();
            mEglCore.release();
        ...
        private void render(long timeStampNanos) {
            //Log.d(TAG, "Rendering timestamp: " + timeStampNanos / 1000000);
            // Render scenes
            ...
            mFrameRateStatist.incomingFrame();
            if (mFrameRateStatist.totalFrameCount() % (int)mDisplayRefreshRate == 0) {
                Log.v(TAG, "MixerThread current speed: "
        ...
        private boolean removeSource(int sourceId) {
            if (!mSources.containsKey(sourceId)) {
                Log.e(TAG, "Source id=" + sourceId + " not exists!");
                return false;
            }
        ...
        private boolean reconfigScene(int sceneId, String config) {
            if (!mScenes.containsKey(sceneId)) {
                Log.e(TAG, "reconfigScene: Scene id not exists!");
                return false;
            }
            //Log.i(TAG, "Reconfig video scene#" + sceneId + ", config: " + config);
        ...
        private void queryScene(int sceneId) {
            String stats = "{}";
            if (!mScenes.containsKey(sceneId)) {
                Log.e(TAG, "queryScene: Scene id" + "(#" + sceneId + ")"+" not exists!");
            } else {
        ...
        private boolean attachDisplaySurface(int sceneId, Surface surface) {
            if (!mScenes.containsKey(sceneId)) {
                Log.e(TAG, "attachDisplaySurface: Scene id not exists!");
                return false;
            }
        ...
        private boolean onDisplaySurfaceChanged(int sceneId, int surfaceFormat, Size surfaceSize) {
            if (!mScenes.containsKey(sceneId)) {
                Log.e(TAG, "onDisplaySurfaceChanged: Scene id not exists!");
                return false;
            }
        ...
        private boolean detachDisplaySurface(int sceneId) {
            if (!mScenes.containsKey(sceneId)) {
                Log.e(TAG, "detachDisplaySurface: Scene id not exists!");
                return false;
            }
        ...
        private boolean addSink(Sink sink) {
            ...
            if (!mScenes.containsKey(sceneId)) {
                Log.e(TAG, "addSink: Scene id not exists!");
                return false;
            }
        ...
        private boolean deleteSink(int sceneId, int sinkId) {
            // TODO:
            if (!mScenes.containsKey(sceneId)) {
                Log.e(TAG, "deleteSink: Scene id not exists!");
                return false;
            }
    ...
    private static class MixerHandler extends Handler implements Choreographer.FrameCallback {
        ...
        public MixerHandler(MixerThread mixer) {
            ...
            // Start choreographer
            Log.i(TAG, "Starting choreographer.");
...
*/
        ...
        public void handleMessage(Message msg) {
            ...
            if (mixer == null) {
                Log.w(TAG, "MixerHandler.handleMessage: weak ref is null");
                return;
            }
            ...
            switch (msg.what) {
                ...
                case MSG_ADD_SCENE : {
                    if (msg.obj instanceof Scene) {
                        ...
                        Log.e(TAG, "Object not a valid string array.");
                    }
    ...
    public static VideoEngine allocateInstance(Context context) {
        if (mInstance == null) {
    ...
    private VideoEngine(Context context) {
        Log.i(TAG, "Constructing VideoEngine singleton object.");
        ...
        Log.i(TAG, "Board name: " + Build.BOARD);
        ...
        if (manager != null) {
            ...
            if (display != null) {
                ...
                Log.i(TAG, "Default display size: " + mDisplaySize.x + "x" + mDisplaySize.y
                        + ", refresh rate: " + mDisplayRefreshRate);
            } else {
                Log.e(TAG, "Get default display failed!");
            }
        } else {
            Log.e(TAG, "Get window service failed!");
        }

        if (mDisplaySize.x == 0 || mDisplaySize.y == 0) {
            Log.w(TAG, "Query display size failed, suppose to 1920x1080.");
            mDisplaySize.set(1920, 1080);
        }

        if (mDisplayRefreshRate < 1.0f || mDisplayRefreshRate > 60.0f) {
            Log.w(TAG, "Query display refresh rate failed, suppose to 60.0.");
        ...
        /*while (!mHdmiInputDevice.isPlugged(HdmiInputDevice.HDMI_IN_PORT_2)) {
            Log.w(TAG, "HDMI-IN 2 NOT PLUGGED.");
            try {
                Thread.sleep(10);
            } catch (Exception e) {
                Log.e(TAG, e.toString());
        ...
        mNoSignalImage = TextRenderer.renderTextAsBitmap(
                "No Signal",
                null, 48,
                100, false,
        ...
        // Start mixer thread
        Log.i(TAG, "Starting video mixer thread.");
    ...
    public int registerDisplaySurface(Surface surface) {
        if (surface == null) {
            Log.e(TAG, "Display surface CAN NOT be null.");
            return -1;
        }
        ...
        Log.e(TAG, "Can not register more display surface, max=" + MAXIMUM_DISPLAY_SURFACE_COUNT);
        return -1;
    }
    ...
    public boolean unregisterDisplaySurface(int surfaceId) {
        ...
        Log.e(TAG, "Can not unregister displayId" + surfaceId);
        return false;
    }

    public boolean addSource(int id, int sourceType, String config) {
        if (mSources.containsKey(id)) {
            Log.e(TAG, "Add source failed! - Duplicate source id=" + id);
            return false;
        }
        ...
        if (MixerHandler.MSG_ADD_SOURCE == mMixerThread.waitForReply()) {
            Log.d(TAG, "Add source successfully!");
        } else {
            Log.e(TAG, "Invalid reply from mixer thread.");
            return false;
        }
    ...
    public boolean removeSource(int sourceId) {
        ...
        if (MixerHandler.MSG_REMOVE_SOURCE == mMixerThread.waitForReply()) {
            Log.d(TAG, "Source id=" + sourceId + " removed successfully!");
        } else {
            Log.e(TAG, "Invalid reply from mixer thread.");
            return false;
        }
    ...
    public boolean attachDisplaySurfaceToSource(int sourceId, int surfaceId) {
        if (!mSources.containsKey(sourceId)) {
            Log.e(TAG, "Source id not exists!");
            return false;
        }

        if (surfaceId < 0 || surfaceId >= MAXIMUM_DISPLAY_SURFACE_COUNT) {
            Log.e(TAG, "Surface id out of range.");
            return false;
        }

        if (mDisplaySurfaces[surfaceId] == null) {
            Log.e(TAG, "Surface with id=" + surfaceId + " hasn't register yet.");
            return false;
        }
    ...
    public boolean detachDisplaySurfaceFromSource(int surfaceId) {
        ...
        if (!mSurfaceToSource.containsKey(surfaceId)) {
            Log.e(TAG, "Can not find source by surface id=" + surfaceId);
            return false;
        }

        int sourceId = mSurfaceToSource.get(surfaceId);
        if (!mSources.containsKey(sourceId)) {
            Log.e(TAG, "detachDisplaySurface: Source id=" + sourceId + " not exists!");
            return false;
        }
    ...
    public boolean createScene(int id, int width, int height, int frameRate) {
        if (mScenes.containsKey(id)) {
            Log.e(TAG, "Create scene failed! - Duplicate scene id=" + id);
            return false;
        }
        ...
        if (width <= 0 || width > MAXIMUM_WIDTH
            ...
            Log.e(TAG, "Invalid parameters for createScene. width="
        ...
        if (MixerHandler.MSG_ADD_SCENE == mMixerThread.waitForReply()) {
            Log.d(TAG, "New scene created successfully!");
        } else {
            Log.e(TAG, "Invalid reply from mixer thread.");
            return false;
        }
    ...
    public boolean onDisplaySurfaceChanged(int surfaceId, int format, int width, int height) {
        ...
        if (!mSurfaceToScene.containsKey(surfaceId)) {
            Log.w(TAG, "No scene attached to surface id=" + surfaceId);
            return true;
        }
    ...
    public boolean detachDisplaySurfaceFromScene(int surfaceId) {
        // Find scene by surface
        if (!mSurfaceToScene.containsKey(surfaceId)) {
            Log.e(TAG, "Can not find scene by surface id=" + surfaceId);
            return false;
        }

        int sceneId = mSurfaceToScene.get(surfaceId);
        if (!mScenes.containsKey(sceneId)) {
            Log.e(TAG, "detachDisplaySurface: Scene id=" + sceneId + " not exists!");
            return false;
        }
    ...
    public boolean deleteScene(int sceneId) {
        if (!mScenes.containsKey(sceneId)) {
            Log.e(TAG, "deleteScene: Scene id=" + sceneId + " not exists!");
            return false;
        }
        ...
        if (MixerHandler.MSG_DELETE_SCENE == mMixerThread.waitForReply()) {
            Log.d(TAG, "Scene deleted successfully!");
        } else {
            Log.e(TAG, "Invalid reply from mixer thread.");
            return false;
        }
    ...
    public boolean reconfigScene(int sceneId, String config) {
        if (!mScenes.containsKey(sceneId)) {
            Log.e(TAG, "reconfigScene: Scene id=" + sceneId + " not exists!");
            return false;
        }
    ...
    public String queryScene(int sceneId) {
        if (!mScenes.containsKey(sceneId)) {
            Log.e(TAG, "Scene id=" + sceneId +" not exists!");
            return null;
        }
    ...
    public boolean createSinkFromScene(int id, int sceneId, String config) {
        if (mSinks.containsKey(id)) {
            Log.e(TAG, "Create sink failed! - Duplicate sink id=" + id);
            return false;
        }
        ...
        if (MixerHandler.MSG_ADD_SINK == mMixerThread.waitForReply()) {
            Log.d(TAG, "New sink created successfully!");
        } else {
            Log.e(TAG, "Invalid reply from mixer thread.");
            return false;
        }
    ...
    public boolean createSinkFromSource(int id, int sourceId, String config) {
        ...
        if (mSources.get(sourceId).addSink(sink)) {
            ...
            Log.e(TAG, "Create video sink from source failed!");
            return false;
        }
    }
    ...
    public boolean deleteSink(int sinkId) {
        if (!mSinks.containsKey(sinkId)) {
            Log.e(TAG, "deleteSink: Sink id=" + sinkId + " not exists!");
            return false;
        }
        ...
        if (sink.getInputType() == Sink.INPUT_TYPE_SCENE) {
            ...
            if (MixerHandler.MSG_DELETE_SINK == mMixerThread.waitForReply()) {
                Log.d(TAG, "Sink deleted successfully!");
            } else {
                Log.e(TAG, "Invalid reply from mixer thread.");
                return false;
            }
            ...
            if (mSources.get(sink.getSourceId()).deleteSink(sink)) {
                ...
                Log.e(TAG, "Delete video sink from source failed!");
                return false;
            }
        }
    ...
    public String querySink(int sinkId) {
        if (!mSinks.containsKey(sinkId)) {
            Log.e(TAG, "querySink: Sink id=" + sinkId + " not exists!");
            return null;
        }
    ...
    public boolean requestKeyFrame(int sinkId) {
        if (!mSinks.containsKey(sinkId)) {
            Log.e(TAG, "requestKeyFrame: Sink id=" + sinkId + " not exists!");
            return false;
        }
